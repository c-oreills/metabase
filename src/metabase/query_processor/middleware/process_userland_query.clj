...
(defn- success-response [{query-hash :hash, :as query-execution} {cached? :cached, :as result}]
  (merge
   (-> query-execution
       add-running-time
       (dissoc :error :hash :executor_id :native :card_id :dashboard_id :pulse_id))
   result
   {:status                 :completed
...
(defn process-userland-query
  ...
  (fn [{{:keys [userland-query?]} :middleware, :as query}]
    (if-not userland-query?
      (qp query)
      (handle-user-query-result (query-execution-info query) (qp query)))))
